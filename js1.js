	...
	$.getJSON('http://smumcdnems04.cdnsrv.jio.com/mumsite.cdnsrv.jio.com/jiotv.data.cdn.jio.com/apis/v1.3/getepg/get?offset=0&channel_id='+id,
		...
		dataa = data;
			...
			for(var i=0; i<response.length; i++){
				...
				var d = new Date(currentTime.getTime() + (currentTime.getTimezoneOffset() + 330)*60000);
...
var response = data.epg;
	for(var i=0; i<response.length; i++){
		...
		var time = (startTime[0]*60*60+startTime[1]*60)*1000;
		var currentTime = new Date();
		var day = new Date(currentTime.getTime() + (currentTime.getTimezoneOffset() + 330)*60000);
		var currTime = day.getTime();
		day.setHours(0,0,0,0);
		var dayTime = day.getTime();
		if((currTime - dayTime) > time){
			var nowShowing = time;
